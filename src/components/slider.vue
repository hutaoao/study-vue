<template>
    <transition>
        <div v-if="isShow" :class="isShow ? 'component-base-sideslider-show' : 'component-base-sideslider-hide'"
             class="component-base-sideslider">
...
<script>
    export default {
        props: ['isShow','passData'],//父组件向子组件传值
        data() {
            return {
                list: [
                    {
        ...
        watch: {
            isShow() {
                if (this.isShow) {
                    this.$emit('open', this.list);//子组件向父组件传值 - 数组
                } else {
        ...
        mounted() {
            console.log(this.passData);//这是父组件传过来的内容 - 数组
        },
        methods: {
            close() {
